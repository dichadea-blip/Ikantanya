...
do
    ...
    local CONFIG = {
        ...
        WHITELIST = {
            ...
            ["Robot Kraken"] = true,
            ["Zombie Shark"] = true,
            ["Dead Zombie Shark"] = true,
            ["Zombie Megalodon"] = true,
            ["Bone Whale"] = true,
...
local ARTIFACT_CONFIG = {
    -- Temple 1: Hourglass Diamond Artifact
    {
        ...
        cframe = CFrame.new(1490.12305, 6.624952, -850.539307, -0.982308805, -4.67861128e-09, -0.187268242, -7.57854224e-09, 1, 1.47694985e-08, 0.187268242, 1.59274283e-08, -0.982308805)
    },
    -- Temple 2: Arrow Artifact
    {
        templeName = "Temple 2",
        targetArtifact = "Arrow Artifact",
        cframe = CFrame.new(883.964233, 6.624952, -360.91275, -0.128746182, 9.21072107e-09, 0.1677582, -4.929768e-09, 1, -9.92803972e-09, -0.1677582, -6.16696871e-09, -0.128746182)
    },
    -- Temple 3: Diamond Artifact
    {
        templeName = "Temple 3",
        targetArtifact = "Diamond Artifact",
        cframe = CFrame.new(1836.77136, 6.624952, -288.573303, 0.25269559, 7.769846e-09, -0.967545807, 3.12285877e-08, 1, 1.61864921e-08, 0.967545807, -3.43053443e-08, 0.25269559)
    },
...
local teleportLocations = {
    { Name = "Kohana Volcano", CFrame = CFrame.new(-572.879456, 22.4521465, 148.355331, -0.5764792, -6.67705606e-08, 0.0919371247, -5.74611505e-08, 1, 1.03905414e-07, -0.0919371247, 9.81825394e-08, -0.5764792) },
    { Name = "Sisyphus Statue",  CFrame = CFrame.new(-3728.21606, -135.074417, -1012.12744, -0.977224171, 7.74980258e-09, -0.212209702, 1.5664e-08, 1, -3.5640408e-08, 0.212209702, -3.81539813e-08, -0.977224171) },
    ...
    { Name = "Tropical Grove",  CFrame = CFrame.new(-2095.34106, 197.17, 3718.08008) },
    ...
    { Name = "Treasure Room",  CFrame = CFrame.new(-3606.34985, -266.57373, -1580.97339, 0.8743415, 1.12141152e-13, -0.0501160324, -1.56847693e-13, 1, -8.88127842e-13, 0.0501160324, 8.94872392e-13, 0.8743415) },
    { Name = "Kohana",  CFrame = CFrame.new(-663.904236, 3.04580712, 718.796875, -0.1007225, -2.14183729e-08, -0.4906783, -1.12300391e-08, 1, -2.03902459e-08, 0.4906783, 9.11752096e-09, -0.1007225) },
    ...
    { Name = "Ancient Jungle", CFrame = CFrame.new(1831.71362, 6.624952, -2.279175, 0.213522509, 1.25553285e-07, -0.976938128, -4.32026184e-08, 1, 1.19074642e-07, 0.976938128, 1.67811702e-08, 0.213522509) },
...
-- ====== AUTO FARM SYSTEM (v9.1 no burst) ======

local spamCompletedThread = nil

--[[ Fungsi Utama ]]--
local function farmCycle() end -- Forward declaration

--[[ Logika Siklus Farm (Satu Kali Jalan) ]]--
farmCycle = function()
    if not isAutoFarmOn then return end -- Pemeriksaan keamanan

    -- Hentikan spammer sebelumnya untuk kebersihan
    ...
    -- 1. Sinyal Awal
    pcall(function()
        -- Equip tool sekali saja di awal untuk mencegah race condition
        task.spawn(function() if equipEvent then equipEvent:FireServer(1) end end)
        task.wait(0.01) -- Beri jeda sedikit setelah equip
        ...
        task.wait(0.01)
        task.spawn(function() if cancelFishingEvent then cancelFishingEvent:InvokeServer() end end)
        task.wait(0.01)
    ...
    -- 2. Mulai spam sinyal "fishingCompleted"
...
--[[ Fungsi untuk Memulai dan Menghentikan Farm ]]--
local function startFarm()
    if not isAutoFarmOn then return end -- Check against the global state
    print("[v9.1 No-Burst Farm] Manual loop farm dimulai.")
    ...
    task.spawn(function()
        while isAutoFarmOn do
            ...
            -- Tunggu waktu yang cukup untuk siklus selesai sebelum memulai lagi.
            -- Nilai 1 detik adalah default dari skrip referensi.
            task.wait(1) 
        end
    end)
end
...
local function stopFarm()
    print("[v9.1 No-Burst Farm] Farm dihentikan.")
    if spamCompletedThread then
        task.cancel(spamCompletedThread)
        spamCompletedThread = nil
    end
...
-- Monitor isAutoFarmOn state and control the farm
task.spawn(function()
    local wasAutoFarmOn = isAutoFarmOn
    print("[v9.1 No-Burst Farm] Monitor loop started. Initial state: " .. tostring(isAutoFarmOn))

    while true do
        if isAutoFarmOn and not wasAutoFarmOn then
            print("[v9.1 No-Burst Farm] State changed to ON - starting farm")
            startFarm()
        elseif not isAutoFarmOn and wasAutoFarmOn then
            print("[v9.1 No-Burst Farm] State changed to OFF - stopping farm")
            stopFarm()
        end
